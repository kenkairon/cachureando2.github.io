...
if(puntos == null){
    ...
    document.getElementById('content').innerHTML = `<h4>La nota para la página es <a class="btn-sm btn-danger">${puntos}</a> y el resultado es ${insuf} <i class="btn btn-outline-danger bi bi-emoji-frown"></i><h4/>`;
}else if (puntos == 5 ){
    document.getElementById('content').innerHTML = `<h4>La nota para la página es <a class="btn-sm btn-danger">${puntos}</a> y el resultado es ${suf} <i class="btn btn-outline-danger bi bi-emoji-expressionless"></i><h4/>`;
}else if (puntos == 6 || puntos <= 7 ){
    document.getElementById('content').innerHTML = `<h4>La nota para la página es <a class="btn-sm btn-primary">${puntos}</a> y el resultado es ${bien} <i class="btn btn-outline-success bi bi-emoji-smile"></i><h4/>`;
}else if (puntos == 8){
    document.getElementById('content').innerHTML = `<h4>La nota para la página es <a class="btn-sm btn-primary">${puntos}</a> y el resultado es ${nota} <i class="btn btn-outline-primary bi bi-emoji-laughing"></i><h4/>`;

}else if (puntos == 9 || puntos == 10 ){
    document.getElementById('content').innerHTML = `<h4>La nota para la página es <a class="btn-sm btn-primary">${puntos}</a> y el resultado es ${sobre} <i class=" btn btn-outline-primary bi bi-emoji-sunglasses"></i><h4/>`;
}else if (puntos >= 11 ){
    document.getElementById('content').innerHTML = `<h4>La nota para la página tiene un rango 1 al 10 <i class="bi bi-calendar-x"></i> la nota <a class="btn-sm btn-danger">${puntos}</a> no es valida. Presiona el link <li class="linea"><a class="mover btn-danger btn-sm" href="preguntas.html">PREGUNTAS</a></li> para volver a intentarlo.<h4/>`;
}else{
